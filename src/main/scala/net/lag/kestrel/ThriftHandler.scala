...
case class QueueTransaction(val name: String, val xid: Long, var timerTask: Option[TimerTask])
...
object ThriftPendingReads {
  ...
  def addPendingRead(sessionId: Int, queue: String, xid: Long): Option[Long] = {
...
trait ThriftPendingReads {
  ...
  def addPendingRead(key: String, xid: Long) = { ThriftPendingReads.addPendingRead(sessionId, key, xid) }
