...
class AliasedQueue(val name: String, @volatile var config: AliasConfig, queueCollection: QueueCollection) {
   ...
   */
  def add(value: Array[Byte], expiry: Option[Time], addTime: Time, sessionDescription: Option[() => String]): Boolean = {
    putItems.getAndIncrement()
    putBytes.getAndAdd(value.length)

    config.destinationQueues.foldLeft(true) { case (result, name) =>
      val thisResult = queueCollection.add(name, value, expiry, addTime, sessionDescription)
      result && thisResult
    }
  }
