...
object TextCodec {
  ...
  val read = readLine(true, "ISO-8859-1") { line =>
    if (line.endsWith(":")) {
      ...
      if (segments.size < 2) throw new ProtocolError("Missing queue name!")
      emit(TextRequest(segments(0).toLowerCase, segments(1), segments.drop(2).toList, Nil))
    }
  }

  private def readData(segments: Seq[String], items: mutable.ListBuffer[Array[Byte]]): Stage = readLine(true, "ISO-8859-1") { line =>
    if (line == "") {
      emit(TextRequest(segments(0).toLowerCase, segments(1), segments.drop(2).toList, items.toList))
    } else {
...
case class TextRequest(command: String, queueName: String, args: List[String], items: List[Array[Byte]])
  ...
  protected final def handle(request: TextRequest) = {
    request.command match {
      case "put" =>
        // put <queue> [expiry]:
        try {
          val expiry = request.args.headOption.map { Time.now + _.toInt.milliseconds }
          var count = 0
          request.items.foreach { item =>
            if (setItem(request.queueName, 0, expiry, item)) count += 1
          }
      ...
      case "get" =>
        // get <queue> [timeout]
        try {
          val timeout = request.args.headOption.map { _.toInt }.getOrElse(0)
          closeAllTransactions(request.queueName)
          getItem(request.queueName, timeout, true, false) { item =>
